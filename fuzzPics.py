...
def startWord(wordFile):
    ''' 
    starting a Word process with a wordFile as input
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o","--output-dir", help="Directory to save crashing image files")
    parser.add_argument("-v","--verbose", help="More info", action="store_true")
    parser.add_argument("-d","--delete-arguments",help="Delete input image files when done", action="store_true")
    ...
    if os.path.exists(args.inputs_dir): inputs_dir = args.inputs_dir
    else:
        logger.error('[!] '+ datetime.now().strftime("%Y:%m:%d::%H:%M:%S") + ' Inputs directory does not exist')
        exit()
